...
const EditTrip: React.FC = () => {
  ...
  const { currentTrip, loadTrip, updateTrip, isLoading, error } = useTripStore();
  ...
  // Charger le déplacement et pré-remplir le formulaire
  useEffect(() => {
    if (id) {
      loadTrip(id);
    }
  ...
  useEffect(() => {
    if (currentTrip) {
      setValue('name', currentTrip.name);
      setValue('destination', currentTrip.destination);
      setValue('purpose', currentTrip.purpose);
      setValue('departureDate', currentTrip.departureDate.split('T')[0]);
      setValue('returnDate', currentTrip.returnDate.split('T')[0]);
      setValue('remarks', currentTrip.remarks || '');
    }
  }, [currentTrip, setValue]);

  const onSubmit = async (data: EditTripFormData) => {
    if (!userProfile || !id) {
      console.error('Profil utilisateur ou ID non disponible');
      return;
    }

    try {
      setIsSubmitting(true);
      // Mettre à jour le déplacement et le remettre en brouillon si c'était soumis
      await updateTrip(id, {
        ...data,
        status: 'draft' // Repasser en brouillon pour permettre les modifications
      });
      navigate(`/trips/${id}`);
    } catch (error) {
      console.error('Erreur lors de la modification du déplacement:', error);
    } finally {
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-8">
            <div className="mb-8">
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">
                Modifier "{currentTrip.name}"
              </h2>
              <p className="text-gray-600 dark:text-gray-300">
                Modifiez les informations de votre déplacement
              </p>
              {currentTrip.status === 'submitted' && (
                <div className="mt-4 p-3 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-lg">
                  <p className="text-sm text-orange-800 dark:text-orange-300">
                    ⚠️ Ce déplacement a été soumis. En le modifiant, il repassera en brouillon et devra être re-soumis.
                  </p>
                </div>
              )}
            </div>
